...
if (!parsedVariants.length) {
    ...
    if (!product || product.trim().length < 3){
       ...
       const trimmedName = product.trim().toLowerCase();
...
if (isNaN(productOfferVal) || productOfferVal < 0 || productOfferVal > 100) {
    return res.status(StatusCodes.BAD_REQUEST).json({
        success: false,
        message: "Product Offer must be a number between 0 and 100"
    });
}
...
const category = req.body.category
    ...
    if (!parsedVariants.length) {
      return res.status(StatusCodes.BAD_REQUEST).json({ success: false, message: Messages.VARIENT_REQUIRED });
        ...
        if (!product || product.trim().length < 3) {
      return res.status(StatusCodes.BAD_REQUEST).json({ success: false, message: "Product name must be at least 3 characters" });
    }

    if (!brand) {
      return res.status(StatusCodes.BAD_REQUEST).json({ success: false, message: "Brand is required" });
    }
 ...
 return res.status(StatusCodes.BAD_REQUEST).json({
   ...
   message:"Category is required"
    ...
    for (let i = 0; i < parsedVariants.length; i++) {
      const v = parsedVariants[i];
      if (!v.dialColor || !v.strapColor) {
        return res.status(StatusCodes.BAD_REQUEST).json({ success: false, message: `Variant ${i + 1}: Dial and Strap color required` });
      }
      if (!v.price || v.price <= 0) {
        return res.status(StatusCodes.BAD_REQUEST).json({ success: false, message: `Variant ${i + 1}: Price must be greater than 0` });
      }
...
if (isNaN(stock) || stock < 0) {
    return res.status(StatusCodes.BAD_REQUEST).json({
        success: false,
        message: `Variant ${i + 1}: Stock must be 0 or more`
    });
}
...
if (!Number.isInteger(stock)) {
    return res.status(StatusCodes.BAD_REQUEST).json({
        success: false,
        message: `Variant ${i + 1}: Stock must be a whole number`
    });
}
...
if (stock > 10000) {
    return res.status(StatusCodes.BAD_REQUEST).json({
        success: false,
        message: `Variant ${i + 1}: Stock cannot exceed 10,000`
    });
}
      if (!v.images || v.images.length < 4) {
        return res.status(StatusCodes.BAD_REQUEST).json({ success: false, message: `Variant ${i + 1}:  4 images required` });
      }
    }
